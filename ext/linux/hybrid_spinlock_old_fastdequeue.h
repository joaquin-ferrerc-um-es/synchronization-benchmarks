...
: );
    ...
    "   srli        %[tmp2], %[ticket], 16\n\t"
    "   andi        %[tmp2], %[tmp2], 0xFF\n\t"
    "   sub         %[tmp3], %[tmp3], %[tmp2]\n\t" // diff = next_ticket - now_serving
    "   andi        %[tmp3], %[tmp3], 0xFF\n\t"
    "   li          %[tmp2], %[qthresh_val]\n\t"
    "   bne         %[tmp3], %[tmp2], 2f\n\t"
    "   andi        %[tmp3], %[ticket], 0xFFFF\n\t" // comprueba tail == 0
    "   bnez        %[tmp3], 2f\n\t"
    "   pause\n\t"
    "   j           1b\n\t"
    "2:\n\t"
    ...
    : [lock_ptr] "r" (lock), [ticket_inc] "r" (0x01000000), [qthresh_val] "I" (_Q_THRESHOLD)
    : "memory" );
...
: );
    ...
    "   andi        %[enqueue], %[enqueue], 0xFF\n\t"
    "   or          %[enqueue], %[enqueue], %[tmp3]\n\t" // enqueue = 0 solo si diff==0 y tail==0
    "   bnez        %[enqueue], 2f\n\t"
    "   add         %[tmp2], %[ticket], %[ticket_inc]\n\t"
    ...
    : "memory" );
    if (!enqueue)
        return 0; /* Ticket adquirido inmediatamente */

    depth = ((ticketval >> 24) - (ticketval >> 16)) & 0xFF;
#else
    /* Implementación genérica en C (Corrección de sintaxis realizada) */
    u32 val = READ_ONCE(*lock);
