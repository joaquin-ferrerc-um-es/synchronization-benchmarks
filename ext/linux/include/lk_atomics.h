...
static inline uint16_t xchg_release16(uint16_t *ptr, uint16_t val) {
    ...
    val = old;
#elif defined(__riscv)
    uintptr_t aligned_addr = (uintptr_t)ptr & ~3UL;
    int shift = ((uintptr_t)ptr & 2) * 8;
    uint32_t mask = 0xFFFFU << shift;
    uint32_t set_val = ((uint32_t)val & 0xFFFFU) << shift;
    uint32_t old_word, tmp;
    ...
    "   bnez        %1, 1b\n"
    : "=&r" (old_word), "=&r" (tmp)
    : "r" (aligned_addr), "r" (~mask), "r" (set_val)
    : "memory");
    val = (old_word >> shift) & 0xFFFFU;
#else
    /* TODO: builtin atomic call */
#endif
...
#else
    uint32_t cycles_h, cycles_l, cycles_h2;
    do {
        asm volatile("rdcycleh %0" : "=r" (cycles_h));
        asm volatile("rdcycle %0" : "=r" (cycles_l));
        asm volatile("rdcycleh %0" : "=r" (cycles_h2));
    } while (cycles_h != cycles_h2);
    cycles = ((u64)cycles_h << 32) | cycles_l;
#endif
    return cycles;
}
...
static inline void __cmpwait_relaxed(volatile void *ptr, unsigned long val)
{
