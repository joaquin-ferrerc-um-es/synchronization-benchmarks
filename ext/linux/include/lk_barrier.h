...
/* Mandatory barriers enforce ordering on both devices (I/O) and memory */
...
/* DMA barriers don't need to enforce ordering on I/O devices, just memory */
#define dma_rmb()   asm volatile("fence r, r"       ::: "memory")
#define dma_wmb()   asm volatile("fence w, w"       ::: "memory")

/* SMP barriers only enforce ordering on normal memory, omitting E/S (I/O) */
...
/* RISC-V relaxed atomic operations do not imply order, full fence needed */
